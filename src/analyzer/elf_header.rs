...
impl Analyzer {
    pub(super) fn elf_header_info_as_json(
        &self,
        elf_header: &elf::file::FileHeader<elf::endian::AnyEndian>,
    ) -> serde_json::Value {
        serde_json::json!({
            "class": elf_class_to_string(&elf_header.class),
            "data": elf_data_to_string(&elf_header.endianness),
            "version": elf_version_to_string(elf_header.version),
            "osabi": elf_osabi_to_string(elf_header.osabi),
            "abi_version": elf_abiversion_to_string(elf_header.abiversion),
            "type": elf_type_to_string(elf_header.e_type),
            "machine": elf_machine_to_string(elf_header.e_machine),
            "entry": format!("0x{:x}", elf_header.e_entry),
            "program_header_table_offset": format!("0x{:x}", elf_header.e_phoff),
            "section_header_table_offset": format!("0x{:x}", elf_header.e_shoff),
            "flags": format!("0x{:x}", elf_header.e_flags),
            "elf_header_size": format!("0x{:x}", elf_header.e_ehsize),
            "program_header_table_entry_size": format!("0x{:x}", elf_header.e_phentsize),
            "program_header_table_entries": format!("0x{:x}", elf_header.e_phnum),
            "section_header_table_entry_size": format!("0x{:x}", elf_header.e_shentsize),
            "section_header_table_entries": format!("0x{:x}", elf_header.e_shnum),
            "section_name_strtab_index": format!("0x{:x}", elf_header.e_shstrndx),
        })
    }
}
