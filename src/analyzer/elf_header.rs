...
impl Analyzer {
    pub(super) fn elf_header_info_as_json(
        ...
        let s = serde_json::json!({
            ...
            "abiversion": elf_abiversion_to_string(elf_file.ehdr.abiversion),
            ...
            "phoff": format!("0x{:x}", elf_file.ehdr.e_phoff),
            "shoff": format!("0x{:x}", elf_file.ehdr.e_shoff),
            "flags": format!("0x{:x}", elf_file.ehdr.e_flags),
            "ehsize": format!("0x{:x}", elf_file.ehdr.e_ehsize),
            "phentsize": format!("0x{:x}", elf_file.ehdr.e_phentsize),
            "phnum": format!("0x{:x}", elf_file.ehdr.e_phnum),
            "shentsize": format!("0x{:x}", elf_file.ehdr.e_shentsize),
            "shnum": format!("0x{:x}", elf_file.ehdr.e_shnum),
            "shstrndx": format!("0x{:x}", elf_file.ehdr.e_shstrndx),
        });
        Ok(s)
    }
}
...
fn elf_type_to_string(elf_type: u16) -> String {
    match elf_type {
        ...
        elf::abi::ET_REL => "RELOCATABLE".to_string(),
        elf::abi::ET_EXEC => "EXECUTABLE".to_string(),
        elf::abi::ET_DYN => "DYNAMIC".to_string(),
